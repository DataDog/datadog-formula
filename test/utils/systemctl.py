...
class Systemctl:
    ...
    def preset_folders(self):
        ...
        if SYSTEMD_PRESET_PATH.endswith(":"):
            ...
            if _preset_folder5:
                yield _preset_folder5
            if _preset_folder6:
                yield _preset_folder6
            if _preset_folderX:
                yield _preset_folderX
    ...
    def user_folders(self):
        ...
        if SYSTEMD_UNIT_PATH.endswith(":"):
            if _user_folder1:
                yield expand_path(_user_folder1)
            if _user_folder2:
                yield expand_path(_user_folder2)
            if _user_folder3:
                yield expand_path(_user_folder3)
            if _user_folder4:
                yield expand_path(_user_folder4)
            if _user_folder5:
                yield expand_path(_user_folder5)
            if _user_folder6:
                yield expand_path(_user_folder6)
            if _user_folder7:
                yield expand_path(_user_folder7)
            if _user_folder8:
                yield expand_path(_user_folder8)
            if _user_folder9:
                yield expand_path(_user_folder9)
            if _user_folderX:
                yield expand_path(_user_folderX)
    ...
    def system_folders(self):
        ...
        if SYSTEMD_UNIT_PATH.endswith(":"):
            if _system_folder1:
                yield _system_folder1
            if _system_folder2:
                yield _system_folder2
            if _system_folder3:
                yield _system_folder3
            if _system_folder4:
                yield _system_folder4
            if _system_folder5:
                yield _system_folder5
            if _system_folder6:
                yield _system_folder6
            if _system_folderX:
                yield _system_folderX
